...
CkReductionMsg *findBestEdge(int nMsg, CkReductionMsg **msgs)
{
    ...
    for (int i = 0; i < nMsg; i++)
    {
        ...
        if (curr_to == -1 && curr_from == -1) // this edge is invalid, no contribution
            continue;

        if (curr_weight >= best_weight) // TODO: this shouldn't really have to be >=... whats wrong?
        {
