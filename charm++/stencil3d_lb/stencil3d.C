...
public:
  ...
  Stencil()
      : iterations(0), imsg(0)
        ...
        ,
        temperature((blockDimX + 2) * (blockDimY + 2) * (blockDimZ + 2), 0.0), new_temperature((blockDimX + 2) * (blockDimY + 2) * (blockDimZ + 2), 0.0)
        // Allocate ghost arrays
        ,
        leftGhost(blockDimY * blockDimZ), rightGhost(blockDimY * blockDimZ), topGhost(blockDimX * blockDimZ), bottomGhost(blockDimX * blockDimZ), frontGhost(blockDimX * blockDimY), backGhost(blockDimX * blockDimY)
  {
    usesAtSync = true;
      ...
      for (int k = 1; k < blockDimZ + 1; ++k)
        for (int j = 1; j < blockDimY + 1; ++j)
          for (int i = 1; i < blockDimX + 1; ++i)
          {
            ...
            new_temperature[index(i, j, k)] = (temperature[index(i - 1, j, k)] + temperature[index(i + 1, j, k)] + temperature[index(i, j - 1, k)] + temperature[index(i, j + 1, k)] + temperature[index(i, j, k - 1)] + temperature[index(i, j, k + 1)] + temperature[index(i, j, k)]) * DIVIDEBY7;
          } // end for
    }
  }
...
public:
  ...
  int procNum(int arrayHdl, const CkArrayIndex &idx)
  {
    ...
    {
      CkAbort("In initial chare mapping: block_rows or block_cols is 0 ? Array size computed as %d x %d, with block dim %d\n", M, N, sqrt_nprocs);
    }
